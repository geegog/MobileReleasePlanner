...
class SafePatterns:
    def __init__(self, app_name, sent_id, clean_sents, unclean_data):
        self.appName = app_name
        self.sentid = sent_id
        self.clean_sentences = clean_sents
        self.unclean_sentences = unclean_data

    def extract_features_analyzing_sent_pos_patterns(self):
        raw_features_sent_patterns, remaining_sents, sent_with_features = self.extract_app_features_with_sentence_patterns()
        raw_features_pos_patterns, sents_features = self.extract_app_features_with_pos_patterns(sent_with_features)
        extracted_app_features = raw_features_sent_patterns + raw_features_pos_patterns

        return sents_features

    def save_extracted_features(self, extracted_features):
        file_path = self.appId.upper() + "_EXTRACTED_APP_FEATURES_"
        if self.extraction_mode.value == ExtractionMode.APP_DESCRIPTION.value:
            file_path = file_path + "DESC.pkl"
        elif self.extraction_mode.value == ExtractionMode.USER_REVIEWS.value:
    ...
    @staticmethod
    def extract_features_with_single_pos_pattern(pattern_1, tag_text):
    ...
    def extract_app_features_with_pos_patterns(self, sent_with_features):
        ...
        for sentence_id in sent_with_features.keys():
            ...
            if len(sent_info['extracted_features']) == 0:
                ...
                for pattern in pos_patterns:
                    ...
                    raw_features = self.extract_features_with_single_pos_pattern(pattern, tag_text)
    ...
    @staticmethod
    def sentence_pattern_case1(tag_text):
        raw_features = []
    ...
    def sentence_pattern_case2(self, tag_text):
        raw_features = []
        ...
        if match is not None:
            ...
            enumeration_words = [w.split('/')[0] for index, w in enumerate(matched_text.split()) if
                                 index not in [0, len(words) - 1] and w.split("/")[1] not in ['.', 'CONJ', 'PRON']]
    ...
    @staticmethod
    def sentence_pattern_case3(tag_text):
        raw_features = []
    ...
    @staticmethod
    def sentence_pattern_case4(tag_text):
        raw_features = []
    ...
    def sentence_pattern_case5(self, tag_text):
        raw_features = []
    ...
    def extract_app_features_with_sentence_patterns(self):
        ...
        for sent_index in range(0, len(self.clean_sentences)):
            ...
            raw_features_case1 = self.sentence_pattern_case1(tag_text)
            ...
            raw_features_case2 = self.sentence_pattern_case2(tag_text)
            ...
            raw_features_case3 = self.sentence_pattern_case3(tag_text)
            ...
            raw_features_case4 = self.sentence_pattern_case4(tag_text)
            ...
            raw_features_case5 = self.sentence_pattern_case5(tag_text)
            ...
            if not sent_pattern_found:
                clean_sents_wo_sent_patterns.append(sent)

            sents_with_extracted_features[sent_index] = {'sentence_text': sent_text, 'clean_sent': sent,
                                                         'extracted_features': sent_features}
